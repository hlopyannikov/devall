<div id="header">
    <div class="profile">
        {if $oUserCurrent}
            <a href="{$oUserCurrent->getUserWebPath()}" class="username"><img src="{$oUserCurrent->getProfileAvatarPath(24)}" alt="{$oUserCurrent->getLogin()}" class="avatar" />{$oUserCurrent->getLogin()}</a>
    ...
    <ul class="pages">
        ...
        {/if}
        -->
        <li {if $sMenuHeadItemSelect==''}class="active"{/if}><a href="{cfg name='path.root.web'}">Рубка</a></li>
        <li {if $sMenuHeadItemSelect==''}class="active"{/if}><a href="{router page=''}">Контакты</a></li>
        <li {if $sMenuHeadItemSelect==''}class="active"{/if}><a href="{router page=''}">Календарь</a></li>
